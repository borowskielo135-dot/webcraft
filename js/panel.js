...
$("registerBtn").addEventListener("click", async () => {
  ...
  if (!name || !email || !pass) return setErr("registerErr", "Wypełnij wszystkie pola.");
  if (pass.length < 6) return setErr("registerErr", "Hasło musi mieć min. 6 znaków.");
  try {
...
async function loadAdminOrders(filter) {
  filter = filter || "ALL";
  $("adminOrders").innerHTML = `<div class="loading-spinner">Ładowanie zamówień...</div>`;
  try {
    ...
    $("adminOrders").innerHTML = `<div class="empty-state"><p>Błąd: ${e.message}</p></div>`;
  }
}
...
function renderAdminStats(orders) {
  ...
  $("adminStats").innerHTML = Object.entries(STATUSES).map(([k,s]) => `
...
function renderAdminOrders(filter) {
  const list = filter === "ALL" ? allOrdersCache : allOrdersCache.filter(o => o.status === filter);
...
function openAdminOrder(id) {
  ...
  $("m-status").addEventListener("change", function() {
    ...
    if (sec) sec.style.display = (this.value==="ACCEPTED"||this.value==="DELIVERED") ? "" : "none";
  });
...
async function saveAdminOrder(id) {
  ...
  try {
    ...
    if (price) { update.officialPrice = price; update.clientNote = note; }
    if (status === "DELIVERED") { update.deliveredUrl = url; update.deliveredFiles = files; }
    ...
    if (price && status !== "DELIVERED" && status !== "REJECTED") {
      await db.collection("orders").doc(id).update({ status: price ? "PRICED" : status });
    }
    closeOrderModal();
