// js/script.js — strona główna (index.html)
...
// ══════════════════════════════════════
// AUTH STATE → pokaż/ukryj przyciski
// ══════════════════════════════════════
let currentUser = null;

auth.onAuthStateChanged(async (user) => {
  currentUser = user;
  const heroAuthRow = document.getElementById("heroAuthRow");
  if (heroAuthRow) {
    heroAuthRow.style.display = user ? "none" : "flex";
  }

  // Sprawdź redirect result po Google login
  try {
...
gsap.to("#heroAuthRow", { opacity: 1, duration: 0.7, delay: 1.4 });
...
function openModal() {
  ...
  if (!currentUser) {
    ...
    // Prefill imię i email
    const nameEl = document.getElementById("f-name");
    const mailEl = document.getElementById("f-email");
    if (nameEl && !nameEl.value) nameEl.value = currentUser.displayName || "";
    if (mailEl && !mailEl.value) mailEl.value = currentUser.email || "";
  }
...
function closeModal() {
  ...
  btn.textContent = "Wyceń mój projekt ✦";
  btn.disabled = false;
...
document.getElementById("submitBtn").addEventListener("click", function () {
  if (this.dataset.step === "send") finalSubmit();
  else submitQuote();
});
...
{"cena_min":number,"cena_max":number,"czas_realizacji":"X-Y dni","pozycje":[{"nazwa":"...","cena":"X–Y zł"}],"uwagi":"2 zdania po polsku"}

Widełki: landing 300-1000 zł, firmowa 700-2500 zł, sklep 1500-3000 zł, redesign 500-1500 zł, aplikacja 5000-15000 zł.`;
  ...
  try {
    ...
    document.getElementById("quoteNote").textContent =
      `⏱ Czas realizacji: ${f.czas} • Skontaktujemy się z Tobą w ciągu 24h.`;
    document.getElementById("quoteBreakdown").innerHTML = "";
    quoteData = { cena_min: f.min, cena_max: f.max };
  }
...
async function finalSubmit() {
  ...
  const priceText = document.getElementById("quotePrice").textContent;

  try {
    await createOrder(
      currentUser.uid,
      currentUser.displayName || document.getElementById("f-name").value.trim(),
      currentUser.email,
      {
  ...
  // Formspree jako backup emailowy
  try {
    await fetch("https://formspree.io/f/xgodpnjk", {
      method: "POST",
      headers: {
    ...
    /* nie blokuj jeśli Formspree padnie */
  }
...
let chatOpen = false;
let chatHistory = [];
const CHAT_SYS = `Jesteś przyjaznym asystentem agencji webowej WebCraft. Odpowiadasz po polsku, krótko (max 3-4 zdania). Ceny: landing 300-1000 zł, firmowa 700-2500 zł, sklep 1500-3000 zł. Czas: landing 3-5 dni, firmowa 7-14 dni, sklep 14-28 dni. Jeśli pyta o wycenę — zachęć do formularza.`;
